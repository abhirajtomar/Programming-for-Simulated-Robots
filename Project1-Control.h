...
public:
    ...
    void step(void *model) {
		...
		PxVec3 goalPosition = getGoalPosition();
		
		//Transforming goal coordinates to bot's frame
		PxVec3 goalPositionTrans = pose.getInverse().transform(goalPosition);

		//Calculating the distance between bot and the goal
		PxReal goalDistance = sqrt(pow(goalPositionTrans.x,2) + pow(goalPositionTrans.y,2) + pow(goalPositionTrans.z,2));

		//cout<<"Transformed Goal Position: "<<goalPositionTrans.x<<","<<goalPositionTrans.y<<","<<goalPositionTrans.z<<endl;

		if(goalDistance < currentGoalRadius){
			takeAction(ACTION_STOP);
			vector<rm3d::Blob> blobs = getCurrentColorBlobs();
			for (int i=0; i<blobs.size(); i++){
				if(blobs[i].getColor() == rm3d::BlobRed){
					cout<<"Goal Color: RED"<<endl;
				}
				else if(blobs[i].getColor() == rm3d::BlobYellow){
					cout<<"Goal Color: Yellow"<<endl;
				}
				else if(blobs[i].getColor() == rm3d::BlobGreen){
					cout<<"Goal Color: Green"<<endl;
				}				
			}
		}

		else{					
			// The algorithm is to make the z coordinate of the goal (in bot's frame) almost equal to 0
			// This alligns the bot towards the goal so that moving forward can take it closer to the goal
			// Since it is difficult to make the z coordinate exactly equal to 0, the margin provides a range for the purpose
			PxReal margin = 0.05;
			if((goalPositionTrans.z > margin && goalPositionTrans.x > 0 ) ){
				cout<<"Turning Right \n";
				takeAction(ACTION_RIGHT);
			}
			else if((goalPositionTrans.z < -margin && goalPositionTrans.x > 0)){
				cout<<"Turning Left \n";
				takeAction(ACTION_LEFT);
			}
			else if (goalPositionTrans.z < margin && goalPositionTrans.z > -margin){
				if(goalPositionTrans.x > 0){
					cout<<"Moving Forward \n";
					takeAction(ACTION_FORWARD);
				}
				else{
					cout<<"Moving Backward \n";
					takeAction(ACTION_BACKWARD);
				}
			}
		}
